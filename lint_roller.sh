...
if [ -z "$(git status --porcelain --untracked-files=no)" ] || [ "$1" == "-f" ]; then
  ...
  autopep8 --in-place --select "$errors" -a --recursive msp2lib/
  autopep8 --in-place --select "$belligerent" -a -a -a -a -a --recursive msp2lib/
  >&2 flake8 --select "$errors$belligerent" msp2lib/
